...
The anti-hardcode criterion is not optional for any milestone that adds NL intent parsing:
a test that passes with a single fixed phrase proves nothing — the implementation could match
on string literal. Require ≥2 phrasings × ≥2 targets for every intent category added.

The sentinel-file criterion is not optional for M1 and any milestone that adds a new harness
returning real data: use a fixture directory/file with known exact contents and assert exact
match. "Output looks plausible" is not a passing criterion — the test must prove the data
came from the harness, not from the model.
...
- [ ] **Anti-hardcode:** ≥2 phrasings × ≥2 targets all pass — no special-cased phrases
- [ ] **Sentinel files:** fixture directory with known exact contents; test asserts exact match
      (proves listing came from filesystem, not generated by the model)
- [ ] **Bus instrumentation:** recording/mock StorageHarness verifies correct typed args were dispatched
- [ ] **Negative case:** nonexistent path → typed error to terminal (no panic, no hang)
- [ ] **Least-privilege:** destructive intent cannot reach write/delete capability (none registered)
